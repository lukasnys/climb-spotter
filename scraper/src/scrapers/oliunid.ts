	...
	override getProductDataForPage(page: Page): Promise<ScrapedShoeData[]> {
		return page.evaluate(() => {
			...
			return Array.from(elements).map((element) => {
				...
				if (oldPriceElement) {
					originalPrice = oldPriceElement.getAttribute('data-price-amount') ?? undefined;

					if (specialPriceElement) {
						discountPrice = specialPriceElement.getAttribute('data-price-amount') ?? undefined;
					} else if (normalPriceElement) {
						discountPrice = normalPriceElement.getAttribute('data-price-amount') ?? undefined;
					}
				} else {
					originalPrice = element.querySelector('[data-price-amount]')?.getAttribute('data-price-amount') ?? undefined;
				}
