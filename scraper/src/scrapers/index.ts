...
import puppeteerCloudflare, { Page } from '@cloudflare/puppeteer';
...
export function hasNextPageAvailable(page: Page, selector: string): Promise<boolean> {
...
function validateAndCreateProduct(data: ScrapedShoeData): Shoe | null {
	...
	if (!parsedData.success) {
		logger.warn(`Invalid product data for product ${data.scrapedName}: ${parsedData.error.toString()}`);
		return null;
	}

	const { url, image, scrapedName, originalPrice, discountPrice } = parsedData.data;
...
export abstract class Scraper {
	...
	constructor(retailer: RetailerKey, env?: Env) {
	...
	async getBrowser() {
		...
		return puppeteerCloudflare.launch(this.env?.MYBROWSER);
	}
	...
	async scrape() {
		...
		try {
			...
			const products = rawProductData.map((data) => validateAndCreateProduct(data)).filter((product) => product !== null);
